...
def markdown_references_image(md_path: Path, image_path: Path) -> bool:
...
def resolve_parent_doc_id_for_image(con, image_path: Path) -> tuple[int, int] | None:
    ...
    # Heuristique 1: sibling direct (meme nom de base .md)
    sibling_md = image_path.with_suffix(".md")
    if sibling_md.exists():
        candidates.append(sibling_md)

    # Heuristique 2: dossier OCR (ex: frelon_ocr_images/img-0.jpeg -> frelon.md)
    parent_name = image_path.parent.name
    if parent_name.endswith("_ocr_images"):
    ...
    for folder in [image_path.parent, image_path.parent.parent]:
        if folder.exists():
            candidates.extend(sorted(folder.glob("*.md")))

    # Fallback: corpus complet
    candidates.extend(sorted(SOURCES_DIR.rglob("*.md")))
...
def is_mostly_french_markdown(md_path: Path) -> bool:
    """Retourne: True si le front matter indique >90 % de français."""
...
def markdown_language_distribution(md_path: Path) -> str | None:
    fields = _parse_front_matter_fields(md_path)
    # TODO passer les `fields` en paramètre au lieu de parser le fichier
...
def pdf_page_count(pdf_path: Path) -> int:
    """Retourne le nombre de pages d'un PDF, sinon -1 si indéterminable."""
    if not pdf_path.exists() or pdf_path.suffix.lower() != ".pdf":
        return -1

    # Essaye d'abord un parseur PDF dédié si disponible.
    reader_cls = None
    try:
    ...
    # Fallback sans dépendance externe: comptage des objets /Type /Page.
    try:
...
def add_file(
    ...
    """Ajoute un fichier déposé manuellement dans la base de données.
    Cas d'utilisation :
    - fichier "inerte" (PDF, Excel, ...): ajout dans la table `source` et `source_document` sans remplir les métadonnées
    - fichier Markdown: ajout dans la table `source` et `source_document` en réutilisant les métadonnées du frontmatter yaml.
    - fichier image:
        - si image utilisée dans un fichier Markdown: ajout dans source_document (car pas encore présent, cf. )
        -
    """
    ...
    if source_path.suffix.lower() in PRIMARY_BINARY_EXTENSIONS:
        ...
        # TODO utiliser les infos du fichier ".source.json"
        if source_path.suffix.lower() == '.pdf':
            ocr_status = 'P'
            page_count = pdf_page_count(source_path)

    if source_path.suffix.lower() in IMAGE_EXTENSIONS:
        parent_doc_id, source_id = resolve_parent_doc_id_for_image(con, source_path)
    ...
    if source_path.suffix.lower() == ".md":
        ...
        if is_mostly_french_markdown(source_path):
            ner_status = 1

    now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")

    # 2) ajout dans la table `source`
    if parent_doc_id is None and source_id is None:
        cur = con.execute(
            """
            INSERT INTO source (
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (file_sig, identifiant_source, titre, date_publication,
             date_consultation, path, auteurs_json, periodes_json,
             isbn, issn, doi, url, langues_value, type_source, page_count,
             categorie, excerpt, resume, ocr_status, now, now),
            )
        source_id = cur.lastrowid
        print(f"[ADDED] fichier ajouté source.id={source_id}: {path}")

    # 3. ajout dans la table `source_document`
    cur = con.execute(
        """
    ...
    print(f"[ADDED] fichier ajouté avec source_document.id={document_id}: {path}")
    return None
